...
C2D-XML 是 Capture2Doc 的规范中间表示。目标流程中，页面解析器先保存逐页内部结果，最终输入被确认且所需结果齐备后，VLM 按确认页序生成更新包，文档组装器形成 C2D-XML；输出层再通过确定性转换生成以下目标格式：
...
2026-09-05 流程核对：逐页上传/OCR 与最终文档确认已解耦，完整时序见 [扫描 Pipeline](capture_pipeline.md)。允许离线先采集并冻结本地最终页序，关联服务端 ID、上传引用页后再提交会话；未连接提示和本地完成不代表 OCR 或 XML 完成。上传回调顺序不决定文档顺序；不可变页面身份、内容摘要和最终引用集合由业务层管理，不加入本规范。当前 XML 校验和组装模块已实现，本地 CLI 已接入模型续写、动态预算和恢复；单图 GPU 验证见 [服务端 Pipeline](server_pipeline.md)，HTTP 和 Renderer 尚未接通；本次流程调整不变更 v0.1 标签、属性或颜色契约。
