...
2026-09-05 流程核对：逐页上传/OCR 与最终文档确认已解耦，完整时序见 [扫描 Pipeline](capture_pipeline.md)。上传回调顺序不决定文档顺序；页面及内容版本、最终引用集合由业务层管理，不加入本规范。当前 XML 校验和组装模块已实现，但真实模型续写、会话和 Renderer 尚未接通；本次流程调整不变更 v0.1 标签、属性或颜色契约。
