...
2026-09-05 流程核对：图片接收/OCR 与最终文档确认已解耦，完整时序见 [扫描 Pipeline](capture_pipeline.md)。上传回调顺序不决定文档顺序；document_id、不可变 image_id 和最终有序列表由业务层管理，不加入本规范。本地 CLI 已连接模型调用与 XML 组装接口，真实 GPU 联调待执行；HTTP、Android 上传和 Renderer 尚未接通。本次不变更 v0.1 标签、属性或颜色契约。
