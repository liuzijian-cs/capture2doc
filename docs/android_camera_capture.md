...
> 状态：CameraX 低延迟拍照、1280 像素派生图和稳定取景 UI 已实现；本地构建检查通过，2026-09-05 当前离线版本真机人工测试按用户确认通过
...
- 顶部操作仅保留返回，离线时另叠加“未连接服务器 · 照片保存在本地”；候选图片、快门和完成按钮叠加在底部区域，不能参与 Preview 的尺寸测量；
...
- 快门固定在底部中央，完成按钮位于右下方；没有页面或仍存在未安全落盘的保留页时禁用完成；派生图处理失败但安全原图仍存在时不阻止保存提交意图并回任务首页，但任务仍显示本地失败；
- 候选页以圆角方形横向排列在快门上方，新增页追加到末尾并自动露出；
- 单击候选页打开大图，双击立即永久删除，长按拖拽调整顺序并在松手后写入草稿；
- postview 只用于当前进程的低延迟预览，候选显示依次回退到已落盘原图和 1280 派生图；
- 处理中显示小型进度环，失败显示红色图标，旧重拍占位恢复为失败；不展示“已拍 N 张”“保存中”、文件大小、分辨率或耗时等调试文案；
- 顶部不再显示“相机能力测试”，旧批次结果页也不属于正式用户流程。

当前“完成”保存有序页面快照并直接返回任务首页；独立后台上传层等待真实服务端文档 ID 和 1280 派生图，全部引用页上传确认后再发送 finalize。服务端 API 与 OCR 业务队列待按客户端协议提案接入，见 [扫描 Pipeline](capture_pipeline.md)。
...
该文件只是通用图像产物，不是 OCR 结果。postview 出现、原图落盘和派生图就绪是不同触发点；当前上传适配器必须等待 1280 派生图有效可用且服务端文档 ID 已关联，不能以缩略图显示代替文件确认。相机可以在派生图未就绪时回任务首页，后台等待派生图，不改用原图上传。
