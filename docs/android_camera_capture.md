...
当前“完成”进入本地页面整理页，不触发网络操作。目标联网流程由相机页管理候选，载荷就绪后逐页提交，服务端接收后立即排入 OCR 队列；完成时发起最终页序提交并返回任务界面，不强制经过独立整理页。具体载荷与未上传页的协调仍待确定，见 [扫描 Pipeline](capture_pipeline.md)。
...
该文件只是通用图像产物，不是 OCR 结果。postview 出现、原图落盘和派生图就绪是不同触发点；未来上传必须等待所选载荷有效可用，不能以缩略图显示代替文件确认。相机可以在派生图未就绪时进入本地整理页，不意味着未来必须改用原图上传。
