...
Android 相机请求约 5MP 的 4:3 原始 JPEG，实际分辨率由 CameraX 根据设备能力选择；客户端额外生成最长边 1280 像素的 OCR 派生图，横图通常为 `1280×960`，竖图通常为 `960×1280`。这里的 1280 是客户端像素长边，不等于 1280 image tokens。
...
2026-09-05 流程边界补充：Android 已实现离线拍摄及上传客户端，但真实服务端 HTTP 链路未接通。目标 [扫描 Pipeline](capture_pipeline.md) 在服务端成功接收页面后立即排 OCR，不等待整份文档拍完；上传载荷已确定为长边不超过 1280 像素的压缩 JPEG 派生图，需真实 documentId 与文件均就绪；未连接时仅本地保存。入队与 Worker 实际执行分开，本文的单图 token/显存记录不证明业务队列已实现，也不把派生图生成视为完成识别。
