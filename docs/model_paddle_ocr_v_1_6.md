...
2026-09-05 流程边界补充：当前 Android 仅生成本地图像，尚未上传。目标 [扫描 Pipeline](capture_pipeline.md) 在服务端成功接收页面后立即排 OCR，不等待整份文档拍完；原图或派生图作为载荷仍待确定。入队与 Worker 实际执行分开，本文的单图 token/显存记录不证明业务队列已实现，也不把派生图生成视为完成识别。
