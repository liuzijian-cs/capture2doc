...
本文描述目标架构，不表示所有组件均已实现。跨端触发条件、编辑与失败案例统一见 [扫描 Pipeline](capture_pipeline.md)；本轮实现和设计演变见 [开发成果与计划](android_progress_and_plan.md)。当前 Android 只运行本地采集和草稿闭环，服务端 Worker 与 XML 模块可独立运行，业务链路尚未接通。
...
flowchart LR
    subgraph Android[Android App]
        A[拍照或未来视频取帧] --> D[本地页面与候选管理]
        D --> P[有效上传载荷就绪]
        D --> F[完成：最终集合与顺序提交]
    end

    subgraph Server[本地服务端]
        E[接收页面与内容版本] --> Q[逐页 OCR 队列]
        Q --> R[有效页面结果]
...
```text
拍摄与候选管理 ── 上传载荷就绪 ── 逐页接收 ── OCR 排队与结果保存
      │                                                   │
      └── 用户完成：最终集合、顺序与有效版本提交 ──┐         │
                                               ▼         ▼
                               接受并冻结最终输入，等待所需结果齐备
                                               │
                                               ▼
                           按最终页序 VLM 组装、跨页合并与内容过滤
                                               │
                                               ▼
                           XML 更新校验、尾块合并、完整文档校验
                                               │
                                               ▼
                                 Lark / Markdown / SiYuan Renderer
```

系统采用两阶段模型，而不是要求单个 VLM 同时完成高分辨率 OCR、跨页理解和严格格式输出。页面解析器负责视觉内容忠实度，通用 VLM 负责多图关联和 C2D 文档结构映射。

目标服务端在成功接收每张页面后立即排入 OCR 队列，结果绑定稳定 `page_id` 和对应内容版本；入队不意味着 GPU 立即执行。用户无需等待 OCR 才继续编辑或决定结束采集。最终提交被接受后，服务端等待最终引用页面的有效结果齐备，才按确认顺序启动 VLM。删除或排序不要求重跑其余页面 OCR；重拍改变内容，需要对应的新结果。
...
目标联网流程中，拍照页直接承担页面候选管理，不强制经过独立整理页。页面载荷就绪后按稳定 `page_id` 幂等上传，成功接收后排入 OCR 队列。点击“完成”发起最终集合与顺序提交，并返回同一文档的任务/对话界面；服务端接受后冻结本次输入。界面区分等待上传/提交、OCR、VLM 组装、校验和导出，不预先宣称 VLM 正在运行。载荷规格、未上传页与最终提交的协调机制仍待联调设计。

当前 Android 代码仍处于本地采集阶段：图片与草稿只写入 App 私有目录，尚未接入会话 API、上传、OCR 队列或任务状态页。该状态必须与上述目标流程分开描述。

当前相机“完成”仍进入本地整理网格，草稿自动保存且可继续编辑；这是实现过渡状态。未来已有部分页面上传时，保存、放弃和恢复会话的语义需要重新设计，不能继续统一提示“尚未上传”。
...
已决定以可重试、可恢复的逐页上传表达“图片流”。以下是旧架构提出的候选数据，不是当前已实现或最终必填字段：
...
`session_id + page_id + SHA-256` 是既有幂等设计候选；摘要、修订号及并发冲突规则待冻结。已确定的约束是：同一页面重拍后不能复用旧内容的 OCR，最终输入必须绑定有效内容版本。Android 尚未生成文档四边形和质量指标，不能要求它提交虚构值；这些字段的可选性和引入时机须随实际能力确定。
...
既有设计倾向简单 HTTP 会话接口；以下路径用于讨论，不能当成可调用服务或已冻结协议：
...
草案以幂等 PUT 上传页面，以 `ordered_page_ids` 表达最终页序。最终提交必须避免重复、跨会话或无法解析到有效内容的页面引用；内容版本绑定、尚未上传页面如何等待，以及提交重试规则仍需确定，不能仅凭有序 ID 数组就认为协议完整。任务状态暂以轮询为既有设计方向，具体字段、鉴权和错误结构尚未实现。
...
2. 上传载荷、配对鉴权、内容版本、最终提交与未上传页协调、保存/放弃及跨进程恢复；页面质量指标随实际能力引入；
3. 页面解析器统一输出接口；
