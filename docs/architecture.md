...
目标服务端在成功接收每张图片后立即排入 OCR 队列，结果绑定所属 document_id 与不可变 image_id；入队不意味着 GPU 立即执行。用户无需等待 OCR 才继续编辑或决定结束采集。最终提交冻结 ordered_image_ids，结果齐备后按该顺序启动 VLM。删除或排序不重跑其余图片 OCR；重拍创建新的 image_id。
...
目标联网流程中，拍照页直接承担页面候选管理，不强制经过独立整理页。图片载荷就绪后按不可变 image_id 幂等上传，成功接收后排入 OCR 队列。点击“完成”提交最终有序 image_id 列表并返回同一文档任务；服务端接受后冻结输入。界面区分等待上传/提交、OCR、VLM 组装、校验和导出。载荷规格、未上传图片与最终提交的协调机制仍待联调设计。
...
- `document_id`、不可变 `image_id` 和图片内容摘要；不建立服务端 page_id；
- `source_mode`（`photo` 或 `video`）和采集时间；
- MIME 类型、宽高、字节数和 SHA-256；
- 原图中的文档四边形、旋转角度和已经应用的变换；
- 清晰度、眩光、遮挡和完整性等质量信号。

当前以 document_id 归属不可变图片，同 image_id 不允许换图；重拍使用新 image_id，最终冻结有序身份列表。旧 session_id/page_id/修订号方案不再作为新服务端实体设计。Android 尚未生成文档四边形和质量指标，不要求它提交虚构值；这些字段的可选性与 HTTP 错误响应待联调确定。

删除采用最终集合语义：客户端无需因本地删除而阻塞拍照流程；被删除的 image_id 不进入最终有序数组，未来服务端可取消未开始的 OCR，并在文档回收阶段清理未被引用的图片与 OCR 结果。
...
```text
POST /v1/documents
PUT  /v1/documents/{document_id}/images/{image_id}
...
草案以幂等 PUT 接收不可变图片，以 ordered_image_ids 表达最终顺序。最终提交必须拒绝重复、跨文档或未知图片引用；同 ID 不同内容应冲突。尚未上传图片如何等待、提交重试、鉴权和错误响应尚未实现。本地 CLI 已使用同样的文档/图片语义，但不是 HTTP 服务。
