...
- [Android / Server 文档任务协议 v1](android_task_protocol.md)：Android 发起的创建、JPEG 上传、最终页序与状态查询旧客户端提案；后端已实现新协议，Android 待适配。
...
- 后续处理方向：相机页直接管理候选，不强制经过独立整理页；允许未连接先拍摄并持续提示；关联真实文档 ID 且 1280 派生图就绪后逐页幂等上传，接收成功即排 OCR，最终输入冻结且所需 OCR 任务进入终态后才按确认页序组装。客户端协议适配已实现，后端 API、OCR 队列、鉴权和流式已实现，Android 仍待按新服务协议适配。
