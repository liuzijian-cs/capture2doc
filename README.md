...
2026-09-05 新增 [服务端文档转换 CLI](docs/server_pipeline.md)：用手机图片清单运行 Paddle OCR → Qwen → 校验后的 C2D-XML，提供单并发模型切换、完整 TXT 提示词、有限纠错和检查点恢复。真实 NVIDIA 模型联调待执行；该 CLI 不代表 Android 上传或 HTTP 业务服务已经接通。
